...
export class App {
  ...
  async openSettings(): Promise<void> {
    ...
    if (apiKeyEl) apiKeyEl.value = this.config.ai.api_key;
    if (endpointEl) endpointEl.value = this.config.ai.api_endpoint;
  ...
  async openFolder(): Promise<void> {
    try {
      ...
      if (folderPath && this.fileExplorer) {
        ...
        // 更新配置中的 last_path
        this.config.last_path = folderPath;
      }
    } catch (error) {
